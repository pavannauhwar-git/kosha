...
## FIX-057 — P2 Optimization: Font Subsetting, WebP Conversion, Recharts Lazy-Loading, and Icon Consolidation

- **Commit:** pending
- **PR:** pending
- **Files modified:** `package.json`, `index.html`, `src/main.jsx`, `src/App.jsx`, `src/pages/Dashboard.jsx`, `src/pages/Monthly.jsx`, `src/pages/About.jsx`, `src/pages/Guide.jsx`, `src/pages/Reconciliation.jsx`, `src/pages/Transactions.jsx`, `scripts/ops/release_candidate_check.mjs`, `scripts/tests/test_splitwise_math.mjs`, and multiple JSX component files for WebP and icon mapping.
- **Lint pass:** ✓ (0 errors, 604 pre-existing warnings)
- **Build pass:** ✓ (built successfully in 38.69s)
- **Automated verify steps:** ✓ `npm run release:candidate-check` passes successfully.
- **Manual verify steps:** PENDING-DEVICE-VERIFICATION — 
  1. Inspect network tab on reload: check that Inter font preloads as wght variable subset format and is ~48kB.
  2. Verify all banners/illustrations load in `.webp` format, confirming no original `.png` references are queried.
  3. Verify that changing pages to Dashboard and Activity works smoothly without missing icons.
- **Discovered out of scope:** Lucide-to-Phosphor automatic conversions in scripts required manual correction of specific icon mismatches (e.g., `TrendingDown` -> `TrendDown`, `Sparkles` -> `Sparkle`, and `History` -> `ClockCounterClockwise`) to pass the Vite build.
- **Notes:** Consolidated icon libraries (removed `lucide-react`, converted all references to `@phosphor-icons/react`), lazy-loaded `recharts` on the dashboard and monthly pages, subset the main Inter variable font to reduce initial bundle size, and compressed/converted all illustrations and banners to high-efficiency WebP format. Corrected npm testing scripts to run matching lint checks.
