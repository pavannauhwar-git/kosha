...
function MenuRow({ icon, label, onClick, destructive = false, disabled = false }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-card
                  text-label font-medium transition-all duration-100 disabled:opacity-60
                  ${destructive
                    ? 'text-expense-text hover:bg-expense-bg active:scale-[0.99]'
                    : 'text-ink hover:bg-kosha-surface-2 active:scale-[0.99]'}`}
    >
    ...
    <div className={`relative ${className}`.trim()}>
      ...
      <button
        ...
        className="w-9 h-9 rounded-full bg-brand-container border border-brand-border/70
                   shadow-card-md flex items-center justify-center overflow-hidden
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className={`absolute z-40 w-64 card p-1.5 ring-1 ring-slate-300/35 shadow-card-lg ${dropUp ? 'bottom-[calc(100%+0.65rem)] left-0' : 'top-[calc(100%+0.65rem)] right-0'}`}
            >
              {/* ── Identity header ───────────────────────────────── */}
